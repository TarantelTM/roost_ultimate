...
public class AChickensnowEntity extends Chicken {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ChickenRoostModEntities.A_CHICKENSNOW.get(), 8, 1, 2));
	}

	public AChickensnowEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ChickenRoostModEntities.A_CHICKENSNOW.get(), world);
	}
	...
	public AChickensnowEntity(EntityType<AChickensnowEntity> type, Level world) {
		...
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Snow Chicken"));
	...
	public static void init() {
		SpawnPlacements.register(ChickenRoostModEntities.A_CHICKENSNOW.get(), SpawnPlacements.Type.ON_GROUND,
