...
public class AChickenRabbitHideEntity extends Chicken {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ChickenRoostModEntities.A_CHICKEN_RABBIT_HIDE.get(), 8, 1, 2));
	}

	public AChickenRabbitHideEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ChickenRoostModEntities.A_CHICKEN_RABBIT_HIDE.get(), world);
	}
	...
	public AChickenRabbitHideEntity(EntityType<AChickenRabbitHideEntity> type, Level world) {
		...
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Rabbit Hide Chicken"));
	...
	public static void init() {
		SpawnPlacements.register(ChickenRoostModEntities.A_CHICKEN_RABBIT_HIDE.get(), SpawnPlacements.Type.ON_GROUND,
