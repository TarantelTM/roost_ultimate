...
public class AChickencarrotEntity extends Chicken {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ChickenRoostModEntities.A_CHICKENCARROT.get(), 8, 1, 2));
	}

	public AChickencarrotEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ChickenRoostModEntities.A_CHICKENCARROT.get(), world);
	}
	...
	public AChickencarrotEntity(EntityType<AChickencarrotEntity> type, Level world) {
		...
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Carrot Chicken"));
	...
	public static void init() {
		SpawnPlacements.register(ChickenRoostModEntities.A_CHICKENCARROT.get(), SpawnPlacements.Type.ON_GROUND,
