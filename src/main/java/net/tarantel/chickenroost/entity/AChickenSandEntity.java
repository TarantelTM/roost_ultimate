...
import net.tarantel.chickenroost.procedures.AChickenSandOnEntityTickUpdateProcedure;
...
public class AChickenSandEntity extends Chicken {
	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("warm_ocean"), new ResourceLocation("lukewarm_ocean"),
			new ResourceLocation("ocean"), new ResourceLocation("desert"));

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ChickenRoostModEntities.A_CHICKEN_SAND.get(), 20, 1, 2));
	}

	public AChickenSandEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ChickenRoostModEntities.A_CHICKEN_SAND.get(), world);
	}

	public AChickenSandEntity(EntityType<AChickenSandEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Sand  Chicken"));
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Blocks.SAND));
	}
	...
	public void baseTick() {
		super.baseTick();
		AChickenSandOnEntityTickUpdateProcedure.execute(

		);
	}

	public static void init() {
		SpawnPlacements.register(ChickenRoostModEntities.A_CHICKEN_SAND.get(), SpawnPlacements.Type.ON_GROUND,
