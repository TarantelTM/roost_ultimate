...
import net.tarantel.chickenroost.procedures.AChickenNetherrackNaturalEntitySpawningConditionProcedure;
...
public class AChickenNetherrackEntity extends Chicken {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER)
					.add(new MobSpawnSettings.SpawnerData(ChickenRoostModEntities.A_CHICKEN_NETHERRACK.get(), 8, 1, 2));
	}

	public AChickenNetherrackEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ChickenRoostModEntities.A_CHICKEN_NETHERRACK.get(), world);
	}

	public AChickenNetherrackEntity(EntityType<AChickenNetherrackEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Netherrack Chicken"));
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(Blocks.NETHERRACK));
	}
	...
	public static void init() {
		SpawnPlacements.register(ChickenRoostModEntities.A_CHICKEN_NETHERRACK.get(), SpawnPlacements.Type.ON_GROUND,
				Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
					int x = pos.getX();
					int y = pos.getY();
					int z = pos.getZ();
					return AChickenNetherrackNaturalEntitySpawningConditionProcedure.execute(world, x, y, z);
				});
	}
