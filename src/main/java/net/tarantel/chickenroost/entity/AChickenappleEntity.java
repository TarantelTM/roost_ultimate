...
public class AChickenappleEntity extends Chicken {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ChickenRoostModEntities.A_CHICKENAPPLE.get(), 8, 1, 2));
	}

	public AChickenappleEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ChickenRoostModEntities.A_CHICKENAPPLE.get(), world);
	}
	...
	public AChickenappleEntity(EntityType<AChickenappleEntity> type, Level world) {
		...
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Apple Chicken"));
	...
	public static void init() {
		SpawnPlacements.register(ChickenRoostModEntities.A_CHICKENAPPLE.get(), SpawnPlacements.Type.ON_GROUND,
