...
import net.tarantel.chickenroost.procedures.AChickenEndstoneNaturalEntitySpawningConditionProcedure;
...
public class AChickenEndstoneEntity extends Chicken {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER)
					.add(new MobSpawnSettings.SpawnerData(ChickenRoostModEntities.A_CHICKEN_ENDSTONE.get(), 5, 1, 2));
	}

	public AChickenEndstoneEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ChickenRoostModEntities.A_CHICKEN_ENDSTONE.get(), world);
	}
	...
	public AChickenEndstoneEntity(EntityType<AChickenEndstoneEntity> type, Level world) {
		...
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Endstone Chicken"));
	...
	public static void init() {
		SpawnPlacements.register(ChickenRoostModEntities.A_CHICKEN_ENDSTONE.get(), SpawnPlacements.Type.ON_GROUND,
				Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
					int x = pos.getX();
					int y = pos.getY();
					int z = pos.getZ();
					return AChickenEndstoneNaturalEntitySpawningConditionProcedure.execute(world, x, y, z);
				});
	}
