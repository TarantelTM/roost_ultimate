...
public class AChickenAcaciaWoodEntity extends Chicken {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(ChickenRoostModEntities.A_CHICKEN_ACACIA_WOOD.get(), 20, 1, 2));
	}

	public AChickenAcaciaWoodEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ChickenRoostModEntities.A_CHICKEN_ACACIA_WOOD.get(), world);
	}
	...
	public AChickenAcaciaWoodEntity(EntityType<AChickenAcaciaWoodEntity> type, Level world) {
		...
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Acacia Chicken"));
	...
	public static void init() {
		SpawnPlacements.register(ChickenRoostModEntities.A_CHICKEN_ACACIA_WOOD.get(), SpawnPlacements.Type.ON_GROUND,
